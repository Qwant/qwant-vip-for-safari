...
define_bump_base_branch() {
  ...
  echo "[?] From which base branch? [1/2]"
  ...
  while [[ $baseBranchOption = "" ]]; do
    ...
    if [ "$baseBranchOption" = "1" ]; then
      ...
      echo "[x] Wrong format. Try again. [1/2]"
      baseBranchOption=
    fi
  done
}
...
bump_and_push() {
  ...
  git add *
  git commit -m "[Qwant VIP for Safari] Bump version to $bumpingToVersionNumber-$bumpingToBuildNumber"
